...
<%@ page language="java" contentType="text/html; charset=UTF-8"
 ...
 <jsp:include page="adminheader.jsp"></jsp:include>
  ...
  <c:if test="${count==0}">
    	...
    	<td class="w3-center" width="10%"><b>가격</b></td>
    	<td class="w3-center" width="20%"><b>관리</b></td>
    
    <c:forEach var="product" items="${gList}">
    			...
    			<form method="post" action="admin_gproductModify" style="display: inline-block;">
    				<input type="hidden" name="pageNum" value="${pageNum}">
    			<input type="hidden" name="num" value="${product.num}">
    			
    			<input type="hidden" name="select" value="agpurchase">
    			...
    			<form method="post" action="deleteUserPro" style="display: inline-block;"><button class="w3-padding-small w3-hover-red
    			 w3-button w3-small w3-black w3-padding-small" type="submit"  >삭제</button>
    				<input type="hidden" name="pageNum" value="${pageNum}">
    			<input type="hidden" name="num" value="${product.num}">
    			<input type="hidden" name="select" value="agpurchase"></form></td>
    ...
    <div class="w3-section">
    	...
    	<c:if test="${startPage>bottomLine}">
    		...
    		<a href="glist?pageNum=${startPage-bottomLine}&select=gprolist" class="w3-bar-item w3-button w3-hover-black">«</a>
    		</c:if>
    		
    		<c:forEach var="i" begin="${startPage }" end="${endPage}">
    		<c:if test="${i!=currentPage }">
    		
    		<a href="glist?pageNum=${i}&select=gprolist" class="w3-bar-item w3-button w3-hover-black">${i}</a>
    		</c:if>
    		<c:if test="${i==currentPage }">
    		
    		
    		<a href="glist?pageNum=${i}&select=gprolist" class="w3-bar-item w3-black w3-button">${i}</a>
